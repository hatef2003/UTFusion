...
private:
    ...
    static void sendJson(quint16 port, const QJsonObject& obj) {
        ...
        sock.disconnectFromHost(); // no waitForDisconnected() to avoid warnings
        sock.close();
    }
    ...
    bool test_singleValidMessage() {
        ...
        // Compose a minimal, valid payload
    ...
    bool test_emptyPixelsAllowed() {
        ...
        // values contains one item with an empty "pixels" array
        QJsonObject obj;
        ...
        if (gotValues[0].size() != 0) return false; // empty pixels
        return true;
    }
};
